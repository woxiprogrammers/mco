...
class PurchaseOrderController extends Controller
{
    ...
    public function editPurchaseOrder(Request $request,$purchaseOrder){
        try{
            ...
            $subTotal = round(($request->quantity * $purchaseOrderComponent['rate_per_unit']),3);
            $cgst_amount = round((($purchaseOrderComponent['cgst_percentage'] * $subTotal) / 100),3);
            $sgst_amount = round((($purchaseOrderComponent['sgst_percentage'] * $subTotal) / 100),3);
            $igst_amount = round((($purchaseOrderComponent['igst_percentage'] * $subTotal) / 100),3);
            $total = round(($subTotal + $cgst_amount + $sgst_amount + $igst_amount),3);
            ...
            $vendorInfo['materials'][] = [
                ...
                'subtotal' => round(($purchaseOrderComponent['quantity'] * $purchaseOrderComponent['rate_per_unit']),3)
            ];
            $iterator = 0;
            if($purchaseOrderComponent['cgst_percentage'] == null || $purchaseOrderComponent['cgst_percentage'] == ''){
                $vendorInfo['materials'][$iterator]['cgst_percentage'] = 0;
        ...
        try{
            if($request->has('grnIds') && !empty($request->grnIds)) {
                ...
                foreach($grnData as $grn) {
                    //purchase_order_transaction_components -> purchase_order_transaction
                    //inventory_components_transfers -> grn
                    PurchaseOrderTransactionBackup::create($grn->toArray());
                    $inventoryComponentTransfers = InventoryComponentTransfers::where('grn','=', $grn->grn)->get();
                    if(!$inventoryComponentTransfers->isEmpty()) {
                        foreach($inventoryComponentTransfers as $inventoryComponentTransfer) {
                            InventoryComponentTransfersBackup::create($inventoryComponentTransfer->toArray());
                            $inventoryComponentTransfer->delete();
                        }
                    }
                    $grn->delete();
                }
        ...
        try{
            if($request->has('grnIds') && !empty($request->grnIds)) {
                ...
                foreach($grnData as $grn) {
                    ...
                    $inventoryComponentTransfersBackup = InventoryComponentTransfersBackup::where('grn', $grnInfo['grn'])->get();

                    if(!$inventoryComponentTransfersBackup->isEmpty()) {
                        foreach($inventoryComponentTransfersBackup as $inventoryComponentTransfers) {
                            $iCTInfo = $inventoryComponentTransfers->toArray();
                            $newICT = InventoryComponentTransfers::firstOrNew(['id' => $inventoryComponentTransfers->id]);
                            ...
                            $newICT->save();
                            $inventoryComponentTransfers->delete();
                        }
                    }
