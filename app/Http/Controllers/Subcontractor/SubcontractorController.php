        ...
        try{
            ...
            for($iterator = 0,$pagination = $request->start; $iterator < $end && $pagination < count($listingData); $iterator++,$pagination++ ){
                if($listingData[$pagination]->transactionStatus->slug == 'cancelled'){
                    $balanceAmountAfterChangeStatus = $listingData[$pagination]->bill->quotation->cancelled_bill_transaction_balance_amount - $listingData[$pagination]->total;
                    if($balanceAmountAfterChangeStatus >= 0){
                        $changeStatusButton = '<a href="javascript:void(0);" class="btn btn-xs green dropdown-toggle" type="button" aria-expanded="true" onclick="openDetails('.$listingData[$pagination]['id'].')">
                                        Approve
                                    </a><a href="javascript:void(0);" class="btn btn-xs green dropdown-toggle" type="button" aria-expanded="true" onclick="openDetails('.$listingData[$pagination]['id'].')">
                                        Delete
                                    </a>';
                    }else{
                        $changeStatusButton = '-';
