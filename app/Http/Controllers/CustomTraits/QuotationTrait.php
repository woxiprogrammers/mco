...
trait QuotationTrait{
        ...
        try{
            ...
            foreach($data['product_id'] as $productId){
                ...
                foreach($data['profit_margins'][$productId] as $id => $percentage){
                    ...
                    $quotationProfitMargin = QuotationProfitMarginVersion::where('quotation_product_id', $quotationProduct->id)
                                                    ->where('profit_margin_id', $id)
                                                    ->first();
        ...
        try{
            ...
            foreach($data['product_id'] as $productId){
                ...
                if($request->has('material_rate') && $request->has('material_unit')){
                    ...
                    $clientSuppliedMaterials = QuotationMaterial::where('quotation_id',$quotation['id'])->whereIn('material_id',$productMaterialsId)->where('is_client_supplied', true)->select('id','rate_per_unit','unit_id')->get();
                    if(count($clientSuppliedMaterials) > 0){
    ...
    public function generateQuotationPdf(Request $request,$quotation,$slug,$summarySlug){
        try{
            ...
            foreach($quotation->quotation_products as $quotationProducts){
                ...
                $quotationProductData[$iterator]['rate'] = round($quotationProducts->rate_per_unit,3);
    ...
    public function getProductEditView(Request $request){
        try{
            ...
            if($quotationProduct == null){
                $responseData = [
                    'message' => 'Not a quotation Material'
                ];
                $status = 201;
