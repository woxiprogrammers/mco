...
$hello.on("change", function(event, path,count){
    ...
    if (typeof path !== "undefined") {
        $.ajax({
            url: "/peticash/peticash-management/purchase/transaction/display-images",
...
function removePurchaseImage(imageId,path,originalId){
    var maxCount = parseInt($('#max_files_count').val());
    maxCount = maxCount +  1;
    $('#max_files_count').val(maxCount);
    $.ajax({
        url: "/peticash/peticash-management/purchase/transaction/delete-temp-product-image",
